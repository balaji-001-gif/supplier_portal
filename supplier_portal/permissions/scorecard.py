...
def query_conditions(user):
    """Return query conditions to filter scorecards by supplier for portal users"""
    if "Supplier Portal User" in frappe.get_roles(user):
        supplier = frappe.db.get_value("User", user, "supplier")
        if supplier:
            return """(`tabSupplier Scorecard`.supplier = '{supplier}')""".format(supplier=supplier)
    return ""


def has_permission(doc, ptype, user):
    """Check if user has permission for a specific scorecard"""
    if "Supplier Portal User" in frappe.get_roles(user):
